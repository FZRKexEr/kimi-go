...
| | kimi-cli | kimi-go |
|---|---|---|
| 实现 | tenacity: 指数退避 + jitter，重试 429/500/502/503/连接超时/空响应，最多 3 次 | ✅ 已实现: `RetryableClient` + `ExponentialBackoff`，支持相同的状态码和指数退避 |

实现细节：
- 初始等待时间: 300ms，最大等待时间: 5s
- 指数基数: 2.0，抖动范围: ±500ms
- 可重试状态码: 429, 500, 502, 503, 504
- 支持网络错误和超时错误的重试
- 可通过配置 `max_retries` 调整最大重试次数
